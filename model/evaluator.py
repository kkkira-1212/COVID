...
from .trainer import to_device as todevice


def anomaly(residual, idx_val):
    mu = residual[idx_val].mean()
    sigma = residual[idx_val].std()
    return np.abs(residual - mu) / (sigma + 1e-8)


def threshold(probs, y, idx_val):
...
def evaluate(residual, y_true, idx_val, idx_test):
...
def infer(
    ...
    if coarse_only:
        ...
        with torch.no_grad():
            ...
            residual_vec = (data_coarse['X_next'] - pred_coarse_all).abs()
            residual = residual_vec.mean(dim=1).cpu().numpy()
